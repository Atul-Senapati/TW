...
export default function TestimonialsCarousel() {
    ...
    return (
      <article
        ...
        className={`w-72 md:w-80 lg:w-[28rem] flex-shrink-0 transform transition-all duration-500 ${
          large ? "scale-100" : "scale-90 opacity-40 "
        }`}
      >
        <div
          className={`bg-neutral-900 rounded-2xl p-6 lg:p-10  h-full border border-neutral-800`}
        >
          <div className="flex items-center gap-4">
            <img
              src={t.avatar}
              alt={t.name}
              className={`w-12 h-12 lg:w-16 lg:h-16 rounded-full object-cover ring-2 ring-amber-200/90`}
            />
            <div className="text-left">
              <div className="text-lg lg:text-2xl font-semibold text-amber-400">
                {t.name}
              </div>
          ...
          <blockquote className="mt-4 text-sm lg:text-base text-neutral-300 leading-relaxed">
            “{t.quote}”
          </blockquote>
        </div>
      </article>
  ...
  return (
    <section className="absolute w-screen pt-16 bg-black text-white z-30">
        ...
        <div className="max-w-5xl mx-auto px-6" ref={containerRef}>
          ...
          <div className="flex justify-center mt-8 gap-4">
            <button
              onClick={handlePrev}
              aria-label="Previous"
              className="w-12 h-12 rounded-full bg-gradient-to-bl from-amber-400/90 to-amber-300/90 text-black flex items-center justify-center shadow hover:scale-105 transform transition"
            >
              ...
              {originals.map((t, i) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full ${
                    i === logicalIndex ? "bg-amber-400" : "bg-amber-400/30"
                  }`}
                />
              ))}
            </div> */}
